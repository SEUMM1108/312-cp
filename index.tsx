...
// 3. 调用通义千问 API (这是和原来最大的不同点)
  ...
  const handleGenerateImage = async () => {
    ...
    // Clean up names for the image header (remove descriptions inside parens)
    ...
    // This allows us to style the "image" independently of the screen view (e.g., full height, specific width)
    const container = document.createElement('div');
    container.style.position = 'absolute';
    container.style.top = '0';
    container.style.left = '-9999px';
    container.style.width = '600px'; // Fixed width for optimal readability in image
    container.style.backgroundColor = '#fdfbfb';
    ...
    // 2. Build the HTML content for the image
    container.innerHTML = `
      <div style="display: flex; flex-direction: column; align-items: center; margin-bottom: 24px;">
        <h1 style="font-family: 'Noto Serif SC', serif; font-size: 2.2rem; margin: 0; color: #2d3748; background: -webkit-linear-gradient(left, #0052d4, #4364f7, #6fb1fc); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">南高312 嗑学家</h1>
        <div style="margin-top: 10px; font-size: 1.2rem; font-weight: 700; color: #4a5568;">${cleanNames}</div>
        <div style="margin-top: 6px; font-size: 0.9rem; color: #718096; background: #e2e8f0; padding: 4px 12px; border-radius: 12px;">
          ${selectedStyle.split(' ')[0]} · ${targetWordCount}字
        </div>
      </div>
      ...
      <div style="margin-top: 30px; text-align: center; color: #cbd5e0; font-size: 0.85rem; font-weight: 600;">
        Generated by AI · SEU Industrial Design Lab
      </div>
    `;
    ...
    try {
      ...
      const canvas = await html2canvas(container, {
        scale: 2, // 2x scale for Retina sharpness
        useCORS: true,
        backgroundColor: null
      });

      // 4. Convert to blob and share/download
      canvas.toBlob(async (blob) => {
        if (blob) {
          ...
          if (navigator.canShare && navigator.canShare({ files: [file] })) {
            ...
            link.download = `南高312_${cleanNames}.png`;
      ...
      <div className="output-section">
        {story && (
          <div className="toolbar">
             <button className="tool-btn image-btn" onClick={handleGenerateImage} disabled={isSharingImage}>
              {isSharingImage ? '🖼️ 生成中...' : '🖼️ 生成长图 (Image)'}
            </button>
